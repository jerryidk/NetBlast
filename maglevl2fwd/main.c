...
static void print_stats(void) {
  ...
  for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
    ...
    printf("\nStatistics for port %u ------------------------------"
           ...
           port_statistics[portid].fwded, port_statistics[portid].dropped);
...
static void print_final_stats(uint64_t start, uint64_t end) {
  ...
  float tx_packets_per_sec =
      total_packets_tx / ((end - start) / rte_get_tsc_hz());
  float rx_packets_per_sec =
      total_packets_rx / ((end - start) / rte_get_tsc_hz());

  printf("\n Final statistics ==============================="
         "\nTotal packets sent: %18" PRIu64 " (Tx packets/sec : %f)"
         "\nTotal packets received: %14" PRIu64 " (Rx packets/sec : %f)"
         "\nTotal packets dropped: %15" PRIu64,
         total_packets_tx, tx_packets_per_sec, total_packets_rx,
         rx_packets_per_sec, total_packets_dropped);
  printf("\n====================================================\n");
...
static void l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid,
  ...
  *((uint64_t *)tmp) = mac;
  ...
  // 0x000000000002 + ((uint64_t)dest_portid << 40);
      ...
      if (l2fwd_dramblast_enabled && nb_rx > 0) {
        ...
        for (j = 0; j < nb_rx; j++) {
          ...
          if ((j + 1) < nb_rx) {
            ...
            // rte_prefetch0(((char *)rte_pktmbuf_mtod(pkts_burst[j + 1], void
            // *)) +
            //               64);
            // rte_prefetch0(((char *)rte_pktmbuf_mtod(pkts_burst[j + 1], void
            // *)) +
...
static int l2fwd_parse_args(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argvopt, short_options, lgopts,
    ...
    case 'm':
      ...
      } else if (!strncmp(optarg, "dramblast", strlen("dramblast"))) {
        printf("Enabing dramblast kv!\n");
