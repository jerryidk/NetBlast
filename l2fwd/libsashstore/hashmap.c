...
// Sashstore hashmap
// empty buckets have zero keys
inline struct sashstore_kv_pair *sashstore_hashmap_insert(struct sashstore_hashmap *map,
				uint8_t *key,
				uint8_t *value)
{
	//uint64_t hash = hash_fn(key);
...
inline struct sashstore_kv_pair* sashstore_hashmap_get(struct sashstore_hashmap *map,
			uint8_t *key)
{
	//uint64_t hash = hash_fn(key);
	uint64_t hash = fnv_1((char*)key, KEY_SIZE);
	for (uint64_t i = 0; i < CAPACITY; ++i) {
		...
		if (!memcmp(pair->key, key, KEY_SIZE)) { // hacky, uses zero key as empty marker
			return pair;
		}
	}
