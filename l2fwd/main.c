...
static void print_stats(void) {
    ...
    printf("\nAggregate statistics ==============================="
           ...
           total_packets_tx, total_packets_rx, total_packets_fwded, total_packets_dropped);

    if (total_packets_fwded > 0) {
        printf("\nCycle per fwd packet: %lu", total_hash_duration / total_packets_fwded);
    }
    printf("\n====================================================\n");
}
...
static inline void l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid, uint64_t mac) {
...
static inline void l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid, unsigned queue_id, unsigned lcore_id) { unsigned dst_port = l2fwd_dst_ports[portid];
    ...
    if (sent) port_statistics[dst_port][lcore_id].tx += sent;
}
...
static void l2fwd_main_loop(void) {
    ...
    uint64_t prev_tsc = 0, diff_tsc, cur_tsc, timer_tsc = 0, start_tsc = 0, end_tsc = 0;
    ...
    for (unsigned i = 0; i < qconf->n_rx_port; i++) {
        RTE_LOG(INFO, L2FWD, " -- lcoreid=%u portid=%u rx_queue=%u\n",
                lcore_id, qconf->rx_port_list[i].port_id, qconf->rx_port_list[i].queue_id);
    }
...
static int l2fwd_parse_args(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argvopt, "p:q:T:m:c:", lgopts, &option_index)) != EOF) {
        ...
        case 'm':
            ...
            else if (!strncmp(optarg, "dramblast", 9)) l2fwd_dramblast_enabled = 1;
            else return -1;
            break;
        case 'c':
        ...
        case 'p':
            ...
            if (l2fwd_enabled_port_mask == 0) return -1;
            break;
        case 'q':
            l2fwd_queues_per_port = strtoul(optarg, NULL, 10);
            if (l2fwd_queues_per_port == 0) return -1;
            break;
        case 'T':
...
int main(int argc, char **argv) {
    ...
    if (l2fwd_parse_args(argc, argv) < 0) rte_exit(EXIT_FAILURE, "Invalid L2FWD arguments\n");
    ...
    for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) l2fwd_dst_ports[portid] = 0;
    last_port = 0;
    ...
    if (nb_ports_in_mask % 2) l2fwd_dst_ports[last_port] = last_port;
    ...
    RTE_ETH_FOREACH_DEV(portid) {
        ...
        for (unsigned q = 0; q < l2fwd_queues_per_port; q++) {
            while (rte_lcore_is_enabled(rx_lcore_id) == 0 ||
                ...
                rx_lcore_id++;
                if (rx_lcore_id >= RTE_MAX_LCORE) rte_exit(EXIT_FAILURE, "Not enough cores\n");
            }
    ...
    unsigned nb_mbufs = RTE_MAX(nb_ports * (nb_rxd + nb_txd + MAX_PKT_BURST +
                                RTE_MAX_LCORE * MEMPOOL_CACHE_SIZE), 8192U);

    l2fwd_pktmbuf_pool = rte_pktmbuf_pool_create("mbuf_pool", nb_mbufs, MEMPOOL_CACHE_SIZE, 0,
    ...
    RTE_ETH_FOREACH_DEV(portid) {
        ...
        for (unsigned q = 0; q < l2fwd_queues_per_port; q++) {
            ...
            if (ret < 0) rte_exit(EXIT_FAILURE, "rte_eth_rx_queue_setup failed port=%u queue=%u\n", portid, q);
        }
    ...
    for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
        struct lcore_queue_conf *qconf = &lcore_queue_conf[lcore_id];
        if (qconf->n_rx_port == 0) continue;

        RTE_ETH_FOREACH_DEV(portid) {
            if ((l2fwd_enabled_port_mask & (1 << portid)) == 0) continue;
            qconf->tx_buffer[portid] = rte_zmalloc_socket("tx_buffer",
                                       RTE_ETH_TX_BUFFER_SIZE(MAX_PKT_BURST), 0, rte_eth_dev_socket_id(portid));
            if (qconf->tx_buffer[portid] == NULL) rte_exit(EXIT_FAILURE, "Cannot allocate buffer\n");

            rte_eth_tx_buffer_init(qconf->tx_buffer[portid], MAX_PKT_BURST);
            /* Error callbacks omitted for brevity, but you can attach them to dropping here */
        }
    }
    ...
    if (l2fwd_dramblast_enabled) dramblast_destroy();
