...
| `-A <n>` | how many `aligned_alloc`/`free` round trips each burst performs. `-1` (default since `docs/REFLECT_PATH.md` §8) hoists the buffer to a per-core allocation made once at start-up; `0` is exactly one pair per burst (default before §8: every earlier figure, and every archived log without `-A`); `n > 0` adds `n` extra pairs. Values kept their meaning; only default moved. Log line `dramblast alloc pairs per burst:` now printed always. `harness.sh matrix` pins `-A 0` (archived fit). Sweeping it prices a round trip on this machine instead of quoting one from a paper. |
