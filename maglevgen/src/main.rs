...
pub fn main() {
    ...
    // 1. Parse command-line arguments to support multiple PCI interfaces
    let args: Vec<String> = env::args().collect();
    if args.len() < 2 {
        eprintln!("Usage: cargo run --example generator <pci_bus_id_1> [pci_bus_id_2] ...");
        process::exit(1);
    }
    let pci_addrs = &args[1..];

    // 2. Set up the atomic flag and interrupt handler for graceful hugepage cleanup
    let running = Arc::new(AtomicBool::new(true));
    let r = running.clone();

    ctrlc::set_handler(move || {
        println!("\n[!] Interrupt received. Signaling threads to shut down and unmap hugepages...");
    ...
    // 3. Spawn a dedicated worker thread for each PCI interface
    for pci_addr in pci_addrs {
        let pci_addr_clone = pci_addr.clone();
        let running_clone = running.clone();

        let handle = thread::spawn(move || {
            run_generator(pci_addr_clone, running_clone);
        });
        handles.push(handle);
    }

    // 4. Wait for all threads to terminate safely
    for handle in handles {
        handle.join().unwrap();
    }

    println!("Shutdown complete. Hugepages freed.");
}

/// The main worker function that drives a single PCI interface
fn run_generator(pci_addr: String, running: Arc<AtomicBool>) {
    ...
    // Allocate a distinct mempool for this specific thread/port
    let pool = Mempool::allocate(NUM_PACKETS, 0).unwrap();

    // pre-fill all packet buffers in the pool with data
    {
    ...
    // Wait a brief moment to ensure links are up
    ...
    while running.load(Ordering::SeqCst) {
        ...
        // update sequence number of all packets
        for p in buffer.iter_mut() {
        ...
        // To exploit full duplex, you must constantly drain the RX queue
        // Otherwise, the NIC's RX rings fill up and it may pause/stall.
        let num_rx = dev.rx_batch(0, &mut rx_buffer, BATCH_SIZE);
        if num_rx > 0 {
            // Processing logic goes here if needed
            rx_buffer.drain(..);
        }
