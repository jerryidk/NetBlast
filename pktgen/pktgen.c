...
static int tx_worker_loop(__rte_unused void *arg) {
    ...
    while (!force_quit) {
        ...
        for (int i = 0; i < BURST_SIZE; i++) {
            ...
            if (client_port_counter > 65000) client_port_counter = 1025;

            /* Custom Payload: Insert Metadata and Timestamp immediately before sending */
...
int main(int argc, char *argv[]) {
    ...
    if (g_payload_size < min_payload) {
        printf("Note: Increasing payload size to %zu bytes to fit custom timestamp header.\n", min_payload);
    ...
    if (nb_workers < 2) rte_exit(EXIT_FAILURE, "Error: Require at least 2 worker cores (1 TX, 1 RX)\n");
    ...
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
        ...
        if (mp == NULL) rte_exit(EXIT_FAILURE, "Cannot init mbuf pool for core %u\n", lcore_id);
    ...
    RTE_ETH_FOREACH_DEV(portid) {
        ...
        RTE_LCORE_FOREACH_WORKER(lcore_id) {
            if (lcore_config[lcore_id].port_id == portid && lcore_config[lcore_id].type == CORE_RX) {
                if (rte_eth_rx_queue_setup(portid, lcore_config[lcore_id].queue_id, RX_RING_SIZE,
        ...
        /* Setup TX Queues */
        RTE_LCORE_FOREACH_WORKER(lcore_id) {
            if (lcore_config[lcore_id].port_id == portid && lcore_config[lcore_id].type == CORE_TX) {
                if (rte_eth_tx_queue_setup(portid, lcore_config[lcore_id].queue_id, TX_RING_SIZE,
                                           rte_eth_dev_socket_id(portid), NULL) < 0) {
                    rte_exit(EXIT_FAILURE, "TX queue setup failed\n");
                }
            }
        }

        /* Enable promiscuous mode so loopback traffic doesn't drop due to MAC mismatches */
    ...
    RTE_ETH_FOREACH_DEV(portid) {
        if (tx_queues_per_port[portid] == 0 && rx_queues_per_port[portid] == 0) continue;
